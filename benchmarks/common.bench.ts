...
const ITEM_RANGES = [10_000, 100_000]
const SET_SIZES = [
  100,
  1_000,
  100_000,
...
ITEM_RANGES.forEach((ITEM_RANGE) => {
  SET_SIZES
    .forEach((count) => {
      const items = createArrayWithSteppedIntegers(0, ITEM_RANGE, count)
      const suffix = `([0..${ITEM_RANGE}] * ${count})`

      describe(`${tint('yellow', 'new')} ${suffix}`, () => {
      ...
      describe(`${tint('blue', 'add/has/delete')} ${suffix}`, () => {
        bench(NATIVE_SET_TITLE, () => {
          const set = new Set(items)
          items.forEach((i) => {
            set.add(i)
      ...
      describe(`${tint('red', 'forEach')} ${suffix}`, () => {
        bench(NATIVE_SET_TITLE, () => {
          const set = new Set(items)
          const res: number[] = []
